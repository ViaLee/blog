...
## 初识flutter
Flutter 是 Google 发布的一个用于创建跨平台、高性能移动应用的框架。拥有自己的UI系统布局，支持 iOS、Android、Web、Windows、macOS、Linux、Fuchsia（Google新的自研操作系统）等众多平台。  
Flutter 的Release 包默认是使用 Dart AOT 模式编译的，所以不支持动态化，但 Dart 还有 JIT 或 snapshot 运行方式，这些模式都是支持动态化的。  
> 执行之前是否需要编译  
...
**优点：**
1. 性能高。自绘引擎直接调用系统API绘制UI，性能和原生接近
2. 组件库易维护，UI一致性高  
...
`assets` 静态文件  
`test` 测试代码    
`pubspec.yaml` 配置依赖项文件,类似package.json 

- [Pub](https://pub.dev/) Dart Packages 仓库  
加依赖后需要futter pug get、重启项目
...
## Dart
Dart 强类型语言，同时借鉴了 Java 和 JavaScript。[> 如何看待Dart](https://www.zhihu.com/question/25058236/answer/2795438635)
...
## Widgets
Flutter 中万物皆为Widget。  
和 React 一样，同样遵循 UI = fn(state)， state可以直接赋值，但只能通过setState更新渲染。功能单一原则。  
- StatelessWidget
```dart
