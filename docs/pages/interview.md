...
egg 项目做了哪些基础搭建  
微前端怎么理解，什么场景用微前端  
微前端权限管理控制怎么处理的，登录相关，前端跳转还是后端跳转  
egg 和 nest 区别
monorepo 项目为什么要迁移成 pnpm, 同样节省空间,为什么不用 yarn workspace  
迁移过程中遇到的问题，一些依赖版本不一致，为什么选择统一版本，除了统一版本还有其他办法吗  
说下 dependence 和 devDependence, peerdependence 区别？  
peerDependence 使用场景? 如果环境没有该依赖会怎样
状态管理用的多吗？用过哪些？有关注过最新的这方面的库吗？  
react class 和 hooks 区别，逻辑抽离 hooks 方法和普通 utils 方法区别
umd 是什么，为什么选用它而不选择其他类型，commenJS 区别  
（egg 里面怎么引入文件系统这些）

## 得物

git clone 和 fork、branch 的区别  
虚拟 dom 优点  
react 源码，简述，更新流程  
js 原始类型和包装类型区别  
js 和 ts 区别与关系  
